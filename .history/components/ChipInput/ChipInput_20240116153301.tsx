...
const ChipInput: React.FC<ChipInputProps> = ({ availableSuggestions }) => {
  ...
  const handleInputFocus = () => {
    setIsFocused(true); // Handle input focus state
    setFilteredSuggestions(availableSuggestions.filter(s => !chips.includes(s)));
  };
  const handleInputBlur = () => {
    setIsFocused(false); // Handle input blur state
    setHighlightedChipIndex(null);
  };
    ...
    <div className={styles.chipInput}>
      <div className={styles.chipContainer}>
        ...
        <input
          ...
          onFocus={handleInputFocus} // Bind the focus event
          onBlur={handleInputBlur} // Bind the blur event
        />
      </div>
